    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            for step, data in bar:
                ...
                if (step+1) % 50 == 0:
                 ...
                 #    print("avg_loss: {}".format(avg_loss / 50))
                    ...
                    logger.info(f"epoch: {epoch+1}/{args.epochs} avg_loss: {avg_loss / 50:.5f}")
                    avg_loss = 0
            print(f"avg_loss: {sum(total_val_loss)/len(total_val_loss)}")
            if best_epoch_loss > sum(total_val_loss)/len(total_val_loss):
                ...
                torch.save(model.state_dict(), os.path.join(args.model_save_dir, 'classif_{}_best_model.pt'.format(args.train_version))) ###
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=2e-7) #5e-5
    ...
    parser.add_argument('--logger_path', type=str, default='./log/classif_aug_train_log.log') ###
    ...
    if args.train_version == "train":
        train_loader= pre_data(tokenizer, args, mode='train')   
    else:
        train_loader= pre_data(tokenizer, args, mode='aug')   ###
